...
{
	int i;
  #pragma omp parallel for num_threads(50) firstprivate(n, weights, arrayX, arrayY, xr, yr) private(i)
	for(i = 0; i < n; i++){
			...
			int Ones, int iter, int X, int Y, int Z, int n)
{
  ...
  #pragma omp parallel for num_threads(50) firstprivate(n, arrayX, arrayY, seed) private(i)
   	for(i = 0; i < n; i++){
   		arrayX[i] += 1 + 5*rand2(seed, i);
   		arrayY[i] += -2 + 2*rand2(seed, i);
   	}
  #pragma omp parallel for num_threads(50) firstprivate(n, Ones, iter, Y, Z, arrayX, arrayY, objxy, max_size, index, array) private(i, j, index_X, index_Y)
   	for(i = 0; i<n; i++){
   		for(j = 0; j < Ones; j++){
...
void func2(double *weights, double *probability, int n)
{
	int i;
	double sumWeights=0;
  #pragma omp parallel for num_threads(50) firstprivate(n, weights, probability) private(i)
	for(i = 0; i < n; i++)
   		weights[i] = weights[i] * exp(probability[i]);

  //#pragma omp parallel for num_threads(50) firstprivate(n, weights) private(i) reduction(+:sumWeights)
   	for(i = 0; i < n; i++)
   		sumWeights += weights[i];
  //#pragma omp parallel for num_threads(50) firstprivate(n, weights, sumWeights) private(i)
	for(i = 0; i < n; i++)
...
void func3(double *arrayX, double *arrayY, double *weights, double *x_e, double *y_e, int n)
{
    ...
    int i;
  #pragma omp parallel for num_threads(50) firstprivate(n, arrayX, arrayY, weights) private(i) reduction(+:estimate_x, estimate_y)
	for(i = 0; i < n; i++){
...
{
	int i;
  #pragma omp parallel for num_threads(50) firstprivate(n, u, u1) private(i)
	for(i = 0; i < n; i++){
   		u[i] = u1 + i/((double)(n));
   	}
}
...
{
	int i, j;
  #pragma omp parallel for num_threads(50) firstprivate(n, cfd, u, arrayX, arrayY, x_j, y_j) private(i, j)
	for(j = 0; j < n; j++){
  ...
  #pragma omp parallel for num_threads(50) firstprivate(n, weights, arrayX, arrayY, x_j, y_j) private(i)
	for(i = 0; i < n; i++){
		arrayX[i] = x_j[i];
		arrayY[i] = y_j[i];
		weights[i] = 1/((double)(n));
